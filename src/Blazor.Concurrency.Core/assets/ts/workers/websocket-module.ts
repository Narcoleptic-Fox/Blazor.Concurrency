...
export class WebSocketModule extends BaseWorkerModule {
    ...
    async handleConnect(data: WebSocketConnectionData, id: string): Promise<void> {
        ...
        try {
            ...
            this.sendError(error as Error, id, 'CONNECTION_FAILED');
        }
    }

    async handleSend(data: WebSocketSendData, id: string): Promise<void> {
        ...
        try {
            ...
            this.sendError(error as Error, id, 'SEND_FAILED');
        }
    }

    async handleClose(data: WebSocketCloseData, id: string): Promise<void> {
        ...
        try {
            ...
            this.sendError(error as Error, id, 'CLOSE_FAILED');
        }
    }

    async handleSubscribe(data: { connectionId: string; topic: string }, id: string): Promise<void> {
        ...
        try {
            ...
            this.sendError(error as Error, id, 'SUBSCRIBE_FAILED');
        }
    }

    private setupWebSocketHandlers(connection: WebSocketConnection, initialResponseId: string): void {
        ...
        try {
            ...
            if (connection.reconnectAttempts < connection.maxReconnectAttempts) {
                ...
                this.sendStreamError(
                    `Reconnection attempts exhausted for connection ${connection.id}`,
                    `reconnect_failed_${Date.now()}`
                );
            }
        }
    }

    private processMessage(message: object, messageType: WebSocketMessageType): string | ArrayBuffer | Blob {
        switch (messageType) {
